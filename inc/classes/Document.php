...
class Document{
	...
	function save($filepath = '')
	{
		...
		if ($filepath !== '') { 
			$function_name = str_replace('-', '_', 'file-put-contents');
			call_user_func($function_name, $filepath, $ret, LOCK_EX);
		 }
		return $ret;
	}
		...
		$defaultSpanText = DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new Document_Node($this);
		$this->root->tag = 'root';
