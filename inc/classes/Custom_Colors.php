...
class Custom_Colors {
	...
	public function custom_color_variables() {
		if ( 'd1e4dd' !== strtolower( get_theme_mod( 'background_color', 'D1E4DD' ) ) ) {
			wp_add_inline_style( 'architronix-style', $this->generate_custom_color_variables() );
		}
		...
		foreach ($colors as $color) {
			...
			$color_id = $color['id'];
			$selector = !empty($color['selector'])? $color['selector'] : 'body';

			$color_mod = get_theme_mod( "architronix_{$color_id}", $color['value'] );
			if ( !empty($color_mod) && strtolower( $color['value'] ) != strtolower( $color_mod ) ) {
				$color_mod_rgb = $this->hex2rgb($color_mod, false);
				$color_name = str_replace('_', '-', $color_id);

				$colors_css[$color_id] = "--{$prefix}{$color_name}: {$color_mod};";
				$colors_css["{$color_id}_rgb"] = "--{$prefix}{$color_name}-rgb: {$color_mod_rgb};";
				
				$custom_css[$selector][$color_id] = "--{$prefix}{$color_name}: {$color_mod};";
				$custom_css[$selector]["{$color_id}_rgb"] = "--{$prefix}{$color_name}-rgb: {$color_mod_rgb};";
				
			}
		}
		
		$custom_css = array_filter($custom_css);
		if(!empty($custom_css)){			
			$styles = '';
			foreach ($custom_css as $selector => $vars) {
				if(empty($selector) || empty($vars)) continue;
				$styles .= "{$selector}{ ".implode(' ',$vars)." }";
			}
			if(!empty($styles)){
				wp_add_inline_style( 'architronix', $styles );
			}
		}	
	...
	public function editor_custom_color_variables() {
		wp_enqueue_style(
			'architronix-customizer-preview',
			get_theme_file_uri( 'assets/admin/customizer-preview.css' ),
			array(),
		...
		$background_color = get_theme_mod( 'background_color', 'D1E4DD' );
		if ( 'd1e4dd' !== strtolower( $background_color ) ) {
			wp_add_inline_style( 'architronix-customizer-preview', $this->generate_custom_color_variables( 'editor' ) );
		}
	}
