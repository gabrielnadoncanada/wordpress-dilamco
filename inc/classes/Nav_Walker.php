 ...
 */
	...
	class Nav_Walker extends \Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( $this->has_children && 0 === $depth ) {
				...
				if ( $depth > 0 ) {
					...
					$link_text_before = '<span class="link-hover-animation-1">';
			...
			if(in_array('current-menu-parent', $classes) || in_array('current_page_parent', $classes) || $item->current){
				$link_classes[] = 'active';
			}
			...
			$item_output .= isset( $args->link_before ) ? $args->link_before . $icon_html . $link_text_before. $title. $link_text_after . $args->link_after : '';
		 ...
		 * menu manager, the function will display a fallback menu.
		 *
		...
		public static function fallback( $args ) {
			...
			// The fallback menu.
			$fallback_title_class = $is_logged_in ? '' : ' opacity-0'; // Add opacity class if logged out.
			$fallback_title = !empty($args['fallback_title'])? $args['fallback_title'] : esc_attr__( 'Add a menu', 'architronix' );

			if(!empty($args['items_wrap']) && ($args['items_wrap'] != '%3$s')){
			...
			$link_class = !empty($args['link_class_depth_1'])? $args['link_class_depth_1'] : 'nav-link';

			$fallback_output .= '<li class="nav-item"><a class="'.esc_attr($link_class). $fallback_title_class .'" href="' . ($is_logged_in ? esc_url( admin_url( 'nav-menus.php' ) ) : '#') . '" title="' . $fallback_title . '">' . $fallback_title . '</a></li>';
			if(!empty($args['items_wrap']) && ($args['items_wrap'] != '%3$s')){
				$fallback_output .= '</ul>';
			}
