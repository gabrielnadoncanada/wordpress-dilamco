 ...
 */
function architronix_render_block_core_comment_edit_link( $attributes, $content, $block ) {
	...
	return sprintf(
		'<div %1$s><h6><a class="wp-block-comment-edit-link text-decoration-none link-hover-animation-1" href="%2$s" %3$s>%4$s</a></h6></div>',
		$wrapper_attributes,
		esc_url( $edit_comment_link ),
		$link_atts,
		esc_html__( 'Edit','architronix' )
	);
}
 ...
 */
function architronix_register_block_core_comment_edit_link() {
	register_block_type_from_metadata(
		ABSPATH .'wp-includes/blocks/comment-edit-link',
		array(
			'render_callback' => 'architronix_render_block_core_comment_edit_link',
		)
	);
}

add_action( 'init', 'architronix_register_block_core_comment_edit_link' );