 ...
 */
function architronix_render_block_core_search( $attributes ) {
	...
	$attributes = wp_parse_args(
		$attributes,
		array(
			'label'      => __( 'Search','architronix' ),
			'buttonText' => __( 'Search','architronix' ),
		)
	);
	...
	$label_inner_html = empty( $attributes['label'] ) ? __( 'Search','architronix' ) : wp_kses_post( $attributes['label'] );
	...
	if ( $label->next_tag() ) {
		...
		$label->add_class( 'wp-block-search__label display-4 h4 fw-semibold mb-30 lh-1' );
	...
	if ( $show_button ) {
		$button_classes         = array( 'wp-block-search__button d-none' );
		...
		if ( $button->next_tag() ) {
			...
			if ( 'button-only' === $attributes['buttonPosition'] ) {
				...
				$button->set_attribute( 'aria-label', __( 'Expand search field','architronix' ) );
	...
	if ( $is_expandable_searchfield ) {
		$aria_label_expanded  = __( 'Submit Search','architronix' );
		$aria_label_collapsed = __( 'Expand search field','architronix' );
 ...
 */
function architronix_register_block_core_search() {
	register_block_type_from_metadata(
		ABSPATH .'wp-includes/blocks/search',
		array(
			'render_callback' => 'architronix_render_block_core_search',
		)
	);
}
add_action( 'init', 'architronix_register_block_core_search' );